...
func TestAnchorExternalBrokenIgnoreCachedErrors(t *testing.T) {
	// When IgnoreCachedErrors is true, cached 404s should be reused without retry
	// First run: populate cache with 404
	hT := tTestFileOpts("fixtures/images/imageExternal404.html",
...
func TestTimeoutNotCachedByDefault(t *testing.T) {
	...
	// Second run: should retry (not use cache) because IgnoreCachedErrors is false
...
func TestTimeoutIsCached(t *testing.T) {
	// URLs that timeout should be saved to cache when IgnoreCachedErrors is true
	tSkipShortExternal(t)
	hT := tTestFileOpts("fixtures/links/ip_timeout.html",
		map[string]interface{}{"ExternalTimeout": 1, "EnableCache": true, "IgnoreCachedErrors": true})
	tExpectIssueCount(t, hT, 1)
	tExpectIssue(t, hT, "request exceeded our ExternalTimeout", 1)
	...
	if !ok {
		t.Error("expected timeout to be cached when IgnoreCachedErrors is true, but it wasn't")
	}
...
func TestTimeoutCachedReused(t *testing.T) {
	// When IgnoreCachedErrors is true, cached timeouts should be reused
	tSkipShortExternal(t)
	...
	if hT2.issueStore.MessageMatchCount("hitting") > 0 {
		t.Error("should not retry when timeout is cached and IgnoreCachedErrors is true")
	}
}
...
func TestTimeoutCachedMessage(t *testing.T) {
	...
	// First run: cause and cache a timeout
	hT := tTestFileOpts("fixtures/links/ip_timeout.html",
		map[string]interface{}{"ExternalTimeout": 1, "EnableCache": true})
	tExpectIssueCount(t, hT, 1)

	// Second run: with IgnoreCachedErrors, should use cached timeout and report with "(cached)" message
	hT2 := tTestFileOpts("fixtures/links/ip_timeout.html",
		map[string]interface{}{"EnableCache": true, "IgnoreCachedErrors": true})
	tExpectIssueCount(t, hT2, 1)
